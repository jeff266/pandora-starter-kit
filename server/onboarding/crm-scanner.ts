...
import type { CRMScanResult, PipelineStat, DimensionStat, StageStat, WonLostStat, OwnerStat, AmountDistribution, FieldFillRate } from './types.js';
...
export async function scanCRM(workspaceId: string): Promise<CRMScanResult> {
  const [
    ...
    safe(async (): Promise<PipelineStat[]> => {
      const r = await query(`
        SELECT COALESCE(custom_fields->>'pipeline', 'Default') AS pipeline,
               COUNT(*) AS count,
               COALESCE(SUM(amount), 0) AS total_amount,
               COALESCE(AVG(amount), 0) AS avg_amount,
               AVG(EXTRACT(EPOCH FROM (close_date - created_at))/86400) AS avg_cycle_days
        FROM deals WHERE workspace_id = $1 AND amount IS NOT NULL
    ...
    safe(async (): Promise<StageStat[]> => {
      const r = await query(`
        ...
        FROM deals WHERE workspace_id = $1 AND stage IS NOT NULL
    ...
    safe(async (): Promise<OwnerStat[]> => {
      const r = await query(`
        ...
        FROM deals WHERE workspace_id = $1 AND owner IS NOT NULL
    ...
    safe(async () => {
      const r = await query(`
        SELECT DATE_TRUNC('month', close_date) AS month, COUNT(*) AS count,
               COALESCE(SUM(amount), 0) AS total_amount
        FROM deals WHERE workspace_id = $1 AND close_date IS NOT NULL
    ...
    safe(async (): Promise<FieldFillRate[]> => {
      const r = await query(`
        SELECT key,
               ...
               COUNT(*) * 100.0 / NULLIF((SELECT COUNT(*) FROM deals WHERE workspace_id = $1), 0) AS fill_pct
        FROM deals, jsonb_object_keys(custom_fields) AS key
        WHERE workspace_id = $1 AND custom_fields IS NOT NULL
    ...
    safe(async () => {
      const r = await query(`
        ...
        FROM (
          ...
          WHERE d.workspace_id = $1 GROUP BY d.id
        ) sub
      `, [workspaceId]);
      const row = r.rows[0];
      if (!row) return null;
    ...
    safe(async (): Promise<string[]> => {
      const r = await query(`
        ...
        WHERE workspace_id = $1 AND created_at > NOW() - INTERVAL '30 days'
          AND owner NOT IN (
            SELECT DISTINCT owner FROM deals
            WHERE workspace_id = $1 AND created_at < NOW() - INTERVAL '30 days' AND owner IS NOT NULL
          )
  ...
  return { pipelines, deal_types, record_types, stages, won_lost, owners, close_date_clusters, amount_distribution, custom_field_fill_rates, contacts_per_deal, new_owners, unused_stages };
}
