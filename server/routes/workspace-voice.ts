...
import { configLoader, DEFAULT_VOICE_CONFIG } from '../config/workspace-config-loader.js';
 ...
 * Returns the current voice configuration for the workspace.
 */
router.get('/api/workspaces/:id/config/voice', async (req, res) => {
  try {
    const config = await configLoader.getVoiceConfig(req.params.id);
    res.json(config);
  } catch (err) {
 ...
 * Updates the voice configuration for the workspace. Requires admin access.
 */
router.patch('/api/workspaces/:id/config/voice', (req, res, next) => {
  requireAdmin(req, res, next);
}, async (req, res) => {
  try {
    const workspaceId = req.params.id as string;
    const updated = await configLoader.updateVoiceConfig(workspaceId, req.body);
    res.json(updated);
  } catch (err) {
    res.status(500).json({ error: err instanceof Error ? err.message : String(err) });
  }
 ...
 * Generates a preview of the voice profile.
 */
router.post('/api/workspaces/:id/voice/preview', async (req, res) => {
  try {
    ...
    if (voiceProfile) {
      ...
      profile = await configLoader.getVoiceProfile(req.params.id);
    }
    ...
    const context: VoiceRenderContext = {
      ...
      surface: sampleContext?.surface || 'chat'
    };

    const scenario = sampleContext?.sample_scenario || 'late_quarter_behind';
    const sampleOutputBefore = scenario in SAMPLE_OUTPUTS ? SAMPLE_OUTPUTS[scenario] : SAMPLE_OUTPUTS.late_quarter_behind;
 ...
 * Resets the voice configuration to defaults. Requires admin access.
 */
router.post('/api/workspaces/:id/voice/reset', (req, res, next) => {
  requireAdmin(req, res, next);
}, async (req, res) => {
  try {
    const workspaceId = req.params.id as string;
    const reset = await configLoader.updateVoiceConfig(workspaceId, DEFAULT_VOICE_CONFIG);
    res.json(reset);
  } catch (err) {
    res.status(500).json({ error: err instanceof Error ? err.message : String(err) });
  }
