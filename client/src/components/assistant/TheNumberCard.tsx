...
export default function TheNumberCard({ theNumber: n, briefType, deltaMode }: TheNumberCardProps) {
  if (!n) return null;
  ...
  if (briefType === 'quarter_close') {
    return (
      <div style={{ padding: '4px 0' }}>
        ...
        <Row label="Attainment" value={`${n.attainment_pct?.toFixed(0) ?? '—'}%`} valueColor={n.attainment_pct < 65 ? '#F87171' : '#34D399'} />
        <Row label="Gap to quota" value={fmt(n.gap)} />
        <Row label="Pipeline" value={fmt(n.pipeline_total)} sub={`${n.deal_count} deals`} />
        ...
        {n.forecast?.win_rate && <Row label="Win rate" value={`${(n.forecast.win_rate * 100).toFixed(0)}%`} />}
      </div>
    );
  }
  ...
  if (deltaMode || briefType === 'pulse') {
    return (
      <div style={{ padding: '4px 0' }}>
        ...
        {n.attainment_pct != null && <Row label="Attainment" value={`${n.attainment_pct.toFixed(0)}%`} />}
        ...
        {n.gap > 0 && <Row label="Gap" value={fmt(n.gap)} />}
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '4px 0' }}>
      ...
      {n.attainment_pct != null && <Row label="Attainment" value={`${n.attainment_pct.toFixed(0)}%`} />}
      {n.gap > 0 && <Row label="Gap to quota" value={fmt(n.gap)} />}
      {n.coverage_on_gap > 0 && <Row label="Coverage" value={`${n.coverage_on_gap.toFixed(1)}×`} />}
      {n.forecast?.weighted > 0 && <Row label="Weighted forecast" value={fmt(n.forecast.weighted)} />}
      <Row label="Deals" value={String(n.deal_count)} />
      {n.days_remaining != null && <Row label="Days left in quarter" value={String(n.days_remaining)} />}
    </div>
  );
}
