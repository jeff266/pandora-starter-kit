...
const SCHEMA = {
  deals: {
    ...
    columns: [
      ...
      { name: 'created_date', type: 'date', desc: 'Deal creation date' },
      { name: 'owner_email', type: 'text', desc: 'Rep email address' },
      { name: 'owner_name', type: 'text', desc: 'Rep display name' },
      { name: 'account_id', type: 'UUID', fk: 'accounts', desc: 'Linked account' },
      { name: 'pipeline_name', type: 'text', desc: 'Pipeline name (e.g. New Business, Renewals)' },
      ...
      { name: 'days_since_activity', type: 'integer', desc: 'Days since last activity (computed)' },
      { name: 'health_score', type: 'numeric', desc: 'Deal health 0-100 (computed)' },
  ...
  contacts: {
    ...
    columns: [
      ...
      { name: 'email', type: 'text', desc: 'Primary email' },
  ...
  accounts: {
    ...
    columns: [
      ...
      { name: 'revenue', type: 'numeric', desc: 'Annual revenue' },
      { name: 'health_score', type: 'numeric', desc: 'Account health 0-100' },
      { name: 'open_deal_count', type: 'integer', desc: 'Active deals (computed)' },
      { name: 'workspace_id', type: 'UUID', desc: 'Workspace identifier' },
    ],
  },
  activities: {
    ...
    columns: [
      ...
      { name: 'type', type: 'text', desc: 'meeting, call, email, note, task' },
      { name: 'timestamp', type: 'timestamptz', desc: 'When it occurred' },
      { name: 'actor_email', type: 'text', desc: 'Who performed it' },
  ...
  conversations: {
    ...
    columns: [
      ...
      { name: 'title', type: 'text', desc: 'Call title' },
      { name: 'call_date', type: 'timestamptz', desc: 'When the call happened' },
      { name: 'duration_seconds', type: 'integer', desc: 'Call length in seconds' },
      { name: 'summary', type: 'text', desc: 'AI-generated summary' },
      { name: 'sentiment_score', type: 'numeric', desc: 'Sentiment -1 to 1' },
      { name: 'deal_id', type: 'UUID', fk: 'deals', desc: 'Linked deal' },
      { name: 'account_id', type: 'UUID', fk: 'accounts', desc: 'Linked account' },
      { name: 'is_internal', type: 'boolean', desc: 'Internal meeting flag' },
      { name: 'workspace_id', type: 'UUID', desc: 'Workspace identifier' },
    ],
  },
  deal_stage_history: {
...
export default function SQLWorkspace() {
  ...
  const [sql, setSQL] = useState(`SELECT d.name, d.amount, d.stage_normalized,
       d.days_in_stage, d.owner_name, a.name AS account
FROM deals d
