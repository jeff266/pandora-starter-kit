...
// ============================================================================
// Google Calendar OAuth Flow (separate from Drive)
// ============================================================================

/**
 * Initiates Google Calendar OAuth flow - redirects browser to Google
 */
function handleCalendarAuthorize(req: Request, res: Response): void {
  const workspaceId = req.query.workspaceId as string;
  ...
  const clientId = process.env.GOOGLE_CLIENT_ID;
  const callbackUrl = process.env.GOOGLE_CALENDAR_CALLBACK_URL || process.env.GOOGLE_CALLBACK_URL;

  if (!clientId || !callbackUrl) {
    res.status(500).json({ error: "Missing GOOGLE_CLIENT_ID or GOOGLE_CALENDAR_CALLBACK_URL environment variables" });
    return;
  }
  ...
  const redirectUrl = `${AUTHORIZE_URL}?${params.toString()}`;
  logger.info("Redirecting to Google Calendar OAuth", { workspaceId });
  res.redirect(redirectUrl);
}

// GET /api/auth/google-calendar/authorize?workspaceId=xxx
router.get("/calendar/authorize", handleCalendarAuthorize);
...
router.get("/calendar/callback", async (req: Request, res: Response) => {
  ...
  if (oauthError) {
    if (oauthError === "access_denied") {
      logger.warn("User denied Google Calendar OAuth consent");
      res.redirect("/?error=google_calendar_denied");
      return;
    }
    logger.error(`OAuth error from Google Calendar: ${oauthError}`);
    res.status(400).json({ error: oauthError });
    return;
  }
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      logger.error(`Calendar token exchange failed: ${tokenResponse.status}`);
      res.redirect("/?error=google_calendar_token_failed");
      return;
    }

    logger.info("Google Calendar OAuth successful");
    ...
    // Store connection in database with connector_name: 'google-calendar'
    await query(
      `INSERT INTO connections (workspace_id, connector_name, auth_method, credentials, status, created_at, updated_at)
       VALUES ($1, 'google-calendar', 'oauth', $2, 'connected', NOW(), NOW())
       ON CONFLICT (workspace_id, connector_name) DO UPDATE SET
         credentials = $2, status = 'connected', updated_at = NOW()`,
      [workspaceId, JSON.stringify(encrypted)]
    );

    logger.info("Stored Google Calendar connection", { workspaceId });

    // Redirect back to connectors page
    res.redirect(`/workspaces/${workspaceId}/connectors`);
  } catch (err) {
    logger.error(`Calendar token exchange error: ${err instanceof Error ? err.message : String(err)}`);
    res.redirect("/?error=google_calendar_callback_failed");
  }
});
