...
export const BRIEFING_TEMPLATES: AgentTemplateRow[] = [
  ...
  // ── Retrospective Route Agents ─────────────────────────────────────────────
  // Deterministic pipeline-mode agents used by the 3-phase retro architecture.
  // Fixed skill sequence, hypothesis-guided synthesis, on-demand only.
  {
    id: 'coverage-agent',
    name: 'Coverage Analysis Agent',
    description: 'Layer 1 retrospective: was the miss structural — did the team enter the quarter undercovered?',
    icon: '📉',
    category: 'analysis',
    defaults: {
      skills: ['pipeline-coverage', 'forecast-rollup'],
      audience: { role: 'CRO / RevOps', detail_preference: 'manager' },
      focus_questions: [
        'Was BOQ pipeline coverage sufficient vs the 3x benchmark?',
        'How much revenue depended on in-quarter pipeline creation?',
        'Which reps entered the quarter most undercovered?',
      ],
      data_window: { primary: 'current_quarter', comparison: 'previous_period' },
  ...
  },
  {
    ...
    description: 'Layer 1 retrospective: where did conversion collapse in the funnel?',
    icon: '🔻',
    category: 'analysis',
    defaults: {
      skills: ['pipeline-waterfall', 'stage-velocity-benchmarks'],
      audience: { role: 'CRO / RevOps', detail_preference: 'analyst' },
      focus_questions: [
        'At which stage did conversion deviate most from historical benchmarks?',
        'How did time-in-stage compare to the median for each stage?',
        'Which stage was the biggest source of leakage?',
      ],
      data_window: { primary: 'current_quarter', comparison: 'previous_period' },
      output_formats: ['slack'],
      schedule: { type: 'manual' },
  ...
  {
    id: 'win-loss-agent',
    name: 'Win/Loss Pattern Agent',
    description: 'Layer 2 retrospective: did the right deals win? Where did losses cluster?',
    icon: '🏆',
    category: 'analysis',
    defaults: {
      skills: ['icp-discovery', 'rep-scorecard'],
      audience: { role: 'CRO / Sales Leadership', detail_preference: 'manager' },
      focus_questions: [
        'Did wins concentrate inside or outside ICP?',
        'Did losses cluster around a stage, rep, segment, competitor, or deal size?',
        'What was the common signature in pushed deals?',
      ],
      data_window: { primary: 'current_quarter', comparison: 'previous_period' },
      output_formats: ['slack'],
      schedule: { type: 'manual' },
  ...
  {
    id: 'process-luck-agent',
    name: 'Process vs. Luck Agent',
    description: 'Layer 3 retrospective: was performance earned (REPLICABLE/STRUCTURAL) or circumstantial (LUCKY/UNLUCKY)?',
    icon: '🎲',
    category: 'analysis',
    defaults: {
      skills: ['rep-scorecard', 'conversation-intelligence'],
      audience: { role: 'CRO / RevOps', detail_preference: 'analyst' },
      focus_questions: [
        'Were activity inputs (volume, multithreading, velocity) above or below benchmark?',
        'Were outputs (win rate, ACV, cycle time) above or below benchmark?',
        'Which quadrant does the team fall into: REPLICABLE, UNLUCKY, LUCKY, or STRUCTURAL?',
      ],
      data_window: { primary: 'current_quarter', comparison: 'previous_period' },
      output_formats: ['slack'],
      schedule: { type: 'manual' },
  ...
  {
    id: 'pipeline-health-agent',
    name: 'Forward Pipeline Health Agent',
    description: 'Layer 4 retrospective: are we better or worse positioned entering next quarter?',
    icon: '🔭',
    category: 'analysis',
    defaults: {
      skills: ['pipeline-coverage', 'pipeline-hygiene'],
      audience: { role: 'CRO / RevOps', detail_preference: 'manager' },
      focus_questions: [
        'Is coverage entering next quarter higher or lower than at the equivalent point last quarter?',
        'Did this quarter consume more pipeline than it created?',
        'Are the same systemic issues present in the current open pipeline?',
      ],
      data_window: { primary: 'current_quarter', comparison: 'previous_period' },
      output_formats: ['slack'],
      schedule: { type: 'manual' },
