...
-   **Evidence Infrastructure ("Show the Work"):** Full evidence population across all skills with `SkillEvidence` type (claims, evaluated_records, data_sources, parameters). Evidence builders registered for single-thread-alert, deal-risk-review, pipeline-coverage, and all other skills. Agent runtime accumulates `skillEvidence` map across skill steps, stored in `agent_runs.skill_evidence` JSONB column.
-   **Slack Formatter Upgrade:** `formatWithEvidence()` and `formatAgentWithEvidence()` render structured claim blocks with severity indicators, deal lists, and methodology footers. Backward compatible — falls back to standard formatting when no evidence present.
-   **WorkbookGenerator (Excel Export):** Multi-tab `.xlsx` export service (`server/delivery/workbook-generator.ts`). Tab 1 = Summary & Methodology (metadata, narrative, data sources table, parameters with yellow highlight for configurable, key metrics). Tab 2+ = per-skill data tabs with schema-driven columns, severity-based row coloring (red/yellow/green), frozen headers, auto-filter, claims section. Uses `exceljs`.
-   **Export API Endpoints:** `GET /api/workspaces/:id/skills/:skillId/runs/:runId/export` and `GET /api/workspaces/:id/agents/:agentId/runs/:runId/export` return `.xlsx` workbooks with proper Content-Type and Content-Disposition headers.
