...
-   **Agent Run History + Diff View:** `GET /agents/:agentId/runs` endpoint upgraded with cursor pagination (`before` param), `synthesis_mode`, `synthesis_output` (only populated for `goal_aware` runs — `findings_dump` returns null), `findings_count` (sum of claims across `skill_evidence` JSONB, with fallback to successful skill count), `skills_run` (array of skill IDs), `total_tokens`, `error_message`, and server-computed `trend` (`improving`/`worsening`/`stable`/`null` based on findings_count delta between consecutive runs). `client/src/components/agents/RunHistoryPanel.tsx` — fetches paginated run history, renders each row with status icon (✓/✗/spinner), date, mode badge (teal "goal-aware" / gray "standard"), findings count, trend indicator, and action buttons ([View Output], [Diff ↕], [Retry]); Diff button only visible when current run is `goal_aware` AND a prior `goal_aware` run exists; "Load more" at bottom when `has_more: true`; panel re-mounts via `key={agentId-runHistoryKey}` when a new run completes. `client/src/components/agents/RunDiffView.tsx` — slide-up sheet; parses both synthesis outputs into sections by `## HEADER` splitting (falls back to single "Full Output" section on parse failure); renders side-by-side word-level diff per section using `diffWords()` from `diff` lib; removed text = red-background strikethrough, added text = green highlight; "Not present in [date]" shown in muted italic when a section exists in only one run; "No changes between these runs." empty state when identical. `ViewOutputSheet` (inline in RunHistoryPanel) — renders `synthesis_output` as markdown via ReactMarkdown with styled `h2`, `p`, `strong`, `ol`, `ul` components; shows mode + findings count + token count in subheader. `AgentBuilder.tsx` — imports RunHistoryPanel, adds `runHistoryKey` state that increments after `handleRunNow` succeeds (forcing history re-fetch), panel rendered between Goal Header card and tab bar.
-   **Save as Agent CTA Flow (Wired End-to-End):** `client/src/hooks/useSaveAsAgentTrigger.ts` — pure hook; shows when: ≥5 user turns + skill invoked (via `tool_call_count` or `evidence.skill_evidence_used`) + not dismissed + not already saved. `client/src/components/chat/SaveAsAgentBanner.tsx` — extracted banner component; shows immediately when eligible (loading spinner on button while extraction is in flight, name appears once data lands); slide-up animation; teal left-border visual. `ChatPanel.tsx` updated: imports hook + banner; `extractionLoading` state tracks in-flight status; `modalPendingOpen` flag auto-opens modal if user clicks during loading; banner replaces old inline div; post-save inline toast appears for 6s with "View 'Agent Name' →" link that navigates to `/agents`. `SaveAsAgentModal.tsx`: confidence badge — green "✓ Auto-detected" chip when `confidence === 'high'`; yellow `⚠ Low confidence extraction` warning when `confidence === 'low'`.
-   **Goal-Aware Synthesis Engine (Full Implementation):** DB migration 134 added `synthesis_mode TEXT DEFAULT 'findings_dump' CHECK (IN 'findings_dump','goal_aware')` to `agent_runs`. `AgentDefinition` type extended with optional `goal?: string` and `standing_questions?: string[]` so system agents carry default values. All 6 built-in agent definitions updated with goal mandates and 3–5 standing questions. `runtime.ts` upgraded: `synthesize()` now falls back to registry definition goal/questions when DB agent row has none (registry → DB override chain); `synthesisMode` tracked and persisted via `logAgentRun` `synthesis_mode` column. Four helpers exported as module-level functions for testability: `formatSkillName` (kebab→Title Case), `computeWordBudget` (400 + 80×questions), `compressEvidenceForSynthesis` (critical→warning→info, top 5 per skill, 3000-char hard cap, text fallback), `buildGoalAwareSynthesisPrompt` (STATUS/Q&A/ACTIONS briefing structure with evidence and word budget). 18 unit tests in `server/agents/__tests__/runtime.test.ts` — all pass.
...
-   **Contextual Opening Brief (`server/context/opening-brief.ts`):** Synthesizes role-scoped pipeline data into a Claude-written greeting on every new conversation (no `thread_id` in POST body).
    ...
    - **Target period scoping (briefing.ts + briefing-math.ts):** Closed-won deals filtered by `close_date >= target.period_start AND close_date <= target.period_end` (not `updated_at`). Target selection: `WHERE is_active = true AND period_start <= CURRENT_DATE AND period_end >= CURRENT_DATE ORDER BY amount ASC LIMIT 1`. `getTargetPipelineScope()` returns `{ quota, pipelineScopeId, periodStart, periodEnd, pipelineName, hasTarget }`. Concierge route accepts `?pipeline` query param to scope all deal queries + findings to a specific pipeline. `hasTarget: boolean` added to brief targets — when false, ConciergeView shows pipeline coverage instead of attainment %.
    - **Overnight summary (briefing.ts → ConciergeView.tsx):** `/briefing/concierge` returns `overnightSummary` with `skillsRun` (completed skill_runs in 48h), `findingsSurfaced` (findings found_at in 48h), `autonomousActionsCompleted` (executed actions in 48h, separate COUNT query), `pendingApprovalCount` (open pending actions), `recentActions` (up to 5 executed action titles/timestamps), `lastRunAt`. Frontend renders a "Pandora ran overnight" card on the Strategic tab (non-AE roles) showing counts, recent action lines, and a pending-approval CTA linking to `/settings/automations`.
    - **`/briefing/pipelines` endpoint:** Returns distinct pipelines from deals LEFT JOINed with targets. Shape: `{ pipelines: [{ name, value, hasTarget, targetAmount? }] }`. Powers the "All Data" dropdown.
    ...
    - `renderBriefContext()`: fills named template with all assembled data. Returns formatted context string prepended to the user's message. Includes: Q2 COVERAGE ESTIMATE block (open pipeline weighted + expected rollover + estimated coverage ratio) and PRIORITY FRAME block (cell, focus topics, deprioritize topics from calibrateBriefPriorities).
    ...
    - **Finding preferences (`server/routes/briefing.ts` + migration 175):** `POST /briefing/findings/:findingId/preference` — upserts `watch` or `dismissed` preference per user per finding (dismissed expires in 7 days, watch never expires). `DELETE /briefing/findings/:findingId/preference` removes preference. `assembleOpeningBrief` fetches active preferences and: filters out dismissed findings from topFindings; adds `is_watched: boolean` to each remaining finding. Table: `finding_preferences(id, workspace_id, user_id, finding_id TEXT, preference TEXT, expires_at TIMESTAMPTZ)`.
    - **Q2 coverage estimate:** `assembleOpeningBrief` computes `estimatedQ2Coverage` via SQL: stage-weighted open pipeline (using contract 0.85, proposal 0.70, demo 0.40, evaluation 0.20, discovery 0.10) split into Q2-destined (close_date > quarter_end OR early stage) and Q1 slip probability (30% for late-stage deals closing before quarter_end). Added to `OpeningBriefData` and rendered in `renderBriefContext()`.
    - **Priority frame calibration (`server/context/brief-priorities.ts`):** `calibrateBriefPriorities(temporal, attainmentPct, coverageRatio, hasTarget)` returns a `BriefPriorityFrame` for the current 12-cell (quarter position × attainment band) grid. Cells: early/mid/late/end × below/at/above (plus no_target fallback). Each cell has ordered `primaryTopics`, `suppressTopics`, and `frameLabel`. Called in `assembleOpeningBrief()`, returned as `priorityFrame` in `OpeningBriefData`, rendered in `renderBriefContext()` as PRIORITY FRAME / CURRENT CELL / FOCUS TOPICS / DEPRIORITIZE blocks.
    - **Assign-to-rep action (`server/routes/actions-inline.ts` + `server/actions/executor.ts`):** `POST /actions/assign-to-rep` — `requireWorkspaceAccess` auth; body `{ dealId, taskTitle?, taskNote?, dueDate? }`; resolves deal owner via `deals.owner_email`; creates `action_type = 'assign_to_rep'` record with `approval_status = 'approved'`; executes immediately with `bypassJudgment: true`; returns `{ ok, actionId, assignedTo, taskCreated, operations }`. Executor case for `assign_to_rep`: writes a CRM note on the deal record if CRM is connected; if no CRM, the action record is the audit trail.
...
- `server/orchestrator/report-orchestrator.ts` — reasoning trees generated in parallel, attached to sections before chart suggestion generation
- `server/orchestrator/report-renderer.ts` — renders WHY / WHAT THIS MEANS / ACTION REQUIRED layers in PDF

### Validated Output (Frontera, 2026-03-18, report `2108c107`)
