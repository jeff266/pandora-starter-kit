...
router.post('/:workspaceId/crm-writeback/log/:writeLogId/reverse', requirePermission('connectors.trigger_sync'), async (req, res) => {
  try {
    ...
    const result = await reverseWrite(workspaceId, writeLogId, userId);

    if (result.success) {
      res.json({
        success: true,
        message: 'CRM write reversed successfully',
      ...
      if (result.error?.includes('not found')) {
        statusCode = 404;
      } else if (result.already_reversed || result.undo_window_expired || result.error?.includes('previous value')) {
        statusCode = 400;
      }
