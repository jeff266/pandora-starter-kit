...
const MEDDPICC: MethodologyFramework = {
  id: 'meddpicc',
  label: 'MEDDPICC',
  description: 'Qualification framework focused on verifiable criteria for complex B2B sales',
  vendor: 'PTC (origin)',
  autoDetectPatterns: ['meddic', 'meddpicc', 'economic buyer', 'champion', 'decision criteria', 'identify pain', 'paper process'],
  dimensions: [
    {
      id: 'metrics',
      label: 'Metrics',
...
const BANT: MethodologyFramework = {
  id: 'bant',
  label: 'BANT',
...
const SPICED: MethodologyFramework = {
  id: 'spiced',
  label: 'SPICED',
...
const SPIN: MethodologyFramework = {
  id: 'spin',
  label: 'SPIN Selling',
...
const CHALLENGER: MethodologyFramework = {
  id: 'challenger',
...
const GAP_SELLING: MethodologyFramework = {
  id: 'gap_selling',
  label: 'Gap Selling',
...
const MILLER_HEIMAN: MethodologyFramework = {
  id: 'miller_heiman',
  ...
  autoDetectPatterns: ['miller heiman', 'strategic selling', 'blue sheet', 'buying influences', 'economic buyer', 'user buyer', 'technical buyer', 'coach'],
  dimensions: [
    {
      id: 'economic_buyer',
      label: 'Economic Buyer',
...
const COMMAND_OF_MESSAGE: MethodologyFramework = {
  ...
  autoDetectPatterns: ['command of the message', 'force management', 'positive business outcomes', 'required capabilities', 'pbo', 'metrics of success'],
  dimensions: [
    {
...
const NEAT: MethodologyFramework = {
  id: 'neat',
  label: 'NEAT Selling',
...
const SANDLER: MethodologyFramework = {
  id: 'sandler',
...
export const ALL_FRAMEWORKS: MethodologyFramework[] = [
  MEDDPICC,
  BANT,
  SPICED,
  SPIN,
...
export function getFrameworkById(id: string): MethodologyFramework | null {
  return ALL_FRAMEWORKS.find(f => f.id === id) ?? null;
}
...
export function detectMethodologyFromEvidence(evidence: {
  ...
  for (const framework of ALL_FRAMEWORKS) {
    const fieldMatches = framework.dimensions
      .flatMap(d => d.crmFieldHints)
      .filter(hint =>
        evidence.crmFields.some(f => f.toLowerCase().includes(hint.toLowerCase()))
      ).length;

    const docMatches = framework.autoDetectPatterns
      .filter(pattern =>
        evidence.docKeywords.some(k => k.toLowerCase().includes(pattern.toLowerCase()))
      ).length;
